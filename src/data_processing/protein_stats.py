...
def calculate_mean_sd(data_file, features_file):
    ...
    plt.figure(1)
    plt.suptitle("Mean Scores and 2 S.D. distribution for aaIndex", fontsize=16)

    # plt.xticks(range(len(d.keys())), d.keys(), rotation='vertical')
    plt.subplot(311)
    plt.errorbar(range(len(d.keys())), d_mean['PUNT030101'], [2*i for i in d_sd['PUNT030101']], linestyle='None', marker='*', label='PUNT030101')
    ...
    plt.subplot(312)
    plt.errorbar(range(len(d.keys())), d_mean['KLEP840101'], [2*i for i in d_sd['KLEP840101']], linestyle='None', marker='*',label='KLEP840101')
    # plt.xticks(range(len(d.keys())), ['' for i in range(len(d.keys()))], rotation='vertical')
    plt.tick_params(axis='x', labelbottom='off')
    ...
    plt.subplot(313)
    plt.errorbar(range(len(d.keys())), d_mean['OOBM850102'], [2*i for i in d_sd['OOBM850102']], linestyle='None', marker='*', label='OOBM850102')
    plt.xticks(range(len(d.keys())), d.keys(), rotation='vertical')
